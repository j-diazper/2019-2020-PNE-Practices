...
# Import elements that we will need, Seq for Medium level
import http.server
import http.client
import socketserver
...
class TestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            # If we leave the action on blank or we write /index we will be taken to the initial menu, our index
            if action == "/" or action == "/index":
                contents = Path('index.html').read_text()

            # Once we are in the index menu we can select different options, these are the ones for the basic level

            # /listSpecies, we enter a limit number and the program will give us a list of species from ensembl database
            # the length of the list is equal to the limit we introduced

                if action == "/listSpecies":
                    # This is the basic structure of the html that will be given us back, it is still incomplete
                    ...
                    # We have the arguments we separate them from the action by splitting ?; then we have the couple of
                    # elements, representing the name of the value, limit and the value of the limit itself on the way
                    # Limit= value of the limit, this value is the one we want
                    lim_prm= get_value.split('?')
                    seq_name, index = lim_prm[0].split("=")
