    ...
    def do_GET(self):
        ...
        try:
            if option == "/":
                ...
                ENDP = 'info/assembly/'
                PRMS = '?content-type=application/json'
                content = """<!DOCTYPE html><html lang = "en"><head><meta charset = "utf-8" ><title> Length Chromosome</title >
                </head ><body><h2> The length of the chromosome is: """
                get_value = arguments[1]
                couple = get_value.split('&')
                name, specie = couple[0].split("=")
                index, chromosome = couple[1].split("=")
                conn = http.client.HTTPConnection(SERVER)
                request = ENDP + specie + PRMS
                try:
                ...
                for chromo in chromo_data:
                    ...
                    if name == chromosome:
                        l = chromo["length"]
                        content = f"""<p> - {l} </p><a href="/">Index</a></body></html>"""
                        content += f"""<a href="/">Index </a></body></html>"""
                        cod = 200
                    else:
                        content = """<!DOCTYPE html>
                        ...
                        <body style="background-color: red;">
                        <p>ERROR INVALID VALUE. You must introduce a correct chromosome</p>
                        <a href="/">Index</a></body></html>"""
